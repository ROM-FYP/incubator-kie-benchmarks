...
/**
 * Standalone heap / GC profiler for the OpenSky AirTraffic CEP benchmark.
 *
 ...
 * Symmetric with {@code WikimediaHeapProfileMain} and {@code BinanceHeapProfileMain}.
 *
 * <p>Usage:
 * <pre>
 ...
 * </pre>
 */
public class OpenSkyHeapProfileMain {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (dataset == null || outputFile == null) {
            System.err.println("Usage: OpenSkyHeapProfileMain --dataset <path> " +
        ...
        System.out.printf("[OpenSkyHeapProfile] Loading dataset: %s%n", dataset);
        OpenSkyJsonlLoader loader = new OpenSkyJsonlLoader();
        List<OpenSkyStateVector> events = loader.loadFlat(dataset);
        if (maxEvents < events.size()) events = events.subList(0, maxEvents);
        System.out.printf("[OpenSkyHeapProfile] Loaded %,d events | mode=%s | threads=%d | trials=%d%n",
        ...
        System.out.printf("%n[OpenSkyHeapProfile] SUMMARY:%n");
        ...
        System.out.printf("%n[OpenSkyHeapProfile] Results written to: %s%n", outputFile);
    }

    private static Map<String, Object> runTrial(int t, String mode, List<OpenSkyStateVector> events)
            throws Exception {
        forceGc();
