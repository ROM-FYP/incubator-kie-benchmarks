...
public class Stratifier {
    ...
    public Stratifier(List<RuleMeta> rules) {
        ...
        this.hasCycles = stronglyConnectedComponents.stream().anyMatch(scc -> scc.size() > 1);
    }
    ...
    public List<Set<RuleMeta>> getCyclicComponents() {
        return stronglyConnectedComponents.stream().filter(scc -> scc.size() > 1).collect(Collectors.toList());
    }
    ...
    public String toString() {
        ...
        if (hasCycles) {
            ...
            for (Set<RuleMeta> scc : getCyclicComponents()) {
                sb.append("  - ");
                sb.append(scc.stream().map(RuleMeta::getRuleName).collect(Collectors.joining(", ")));
                sb.append("\n");
            }
        }
