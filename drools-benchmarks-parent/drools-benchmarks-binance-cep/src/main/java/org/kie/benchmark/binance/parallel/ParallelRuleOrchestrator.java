...
public class ParallelRuleOrchestrator {
    ...
    private KieSession buildClusterSession(int clusterId, String clusterDrl, Set<String> symbols, KieBase globalBase) {
        KieServices ks = KieServices.Factory.get();
        KieFileSystem kfs = ks.newKieFileSystem();
        kfs.write("src/main/resources/rules/cluster_" + clusterId + ".drl",
                ks.getResources().newByteArrayResource(clusterDrl.getBytes(StandardCharsets.UTF_8)));

        KieBuilder kb = ks.newKieBuilder(kfs);
        kb.buildAll();
        if (kb.getResults().hasMessages(org.kie.api.builder.Message.Level.ERROR)) {
            throw new RuntimeException("KieBuilder errors for cluster " + clusterId + ": " + kb.getResults());
        }
        ...
        // Register alerts channel to prevent NPE from rules evaluating channels["alerts"].send(...)
        session.registerChannel("alerts", obj -> { /* no-op sink */ });

        // Insert initial fallback facts
        for (String sym : symbols) {
