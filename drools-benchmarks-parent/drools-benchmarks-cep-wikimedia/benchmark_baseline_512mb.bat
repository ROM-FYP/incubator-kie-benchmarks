...
for /L %%W in (1,1,%WARMUP_ITERATIONS%) do (
    ...
    start /affinity %CPU_AFFINITY% /B /WAIT java -Xms%MEMORY_CAP%M -Xmx%MEMORY_CAP%M ^
         -Xlog:gc*:file=selective_routing_results/gc_baseline_512M_w%%W.log:time ^
         -cp %JAR% %MAIN% replay %EVENT_FILE% false ^
         > selective_routing_results/baseline_512M_w%%W.log 2>&1
)

echo [BASELINE 512M] Measurement...
for /L %%I in (1,1,%MEASURE_ITERATIONS%) do (
    echo   Run %%I/%MEASURE_ITERATIONS%
    start /affinity %CPU_AFFINITY% /B /WAIT java -Xms%MEMORY_CAP%M -Xmx%MEMORY_CAP%M ^
         -Xlog:gc*:file=selective_routing_results/gc_baseline_512M_r%%I.log:time ^
         -cp %JAR% %MAIN% replay %EVENT_FILE% false ^
         > selective_routing_results/baseline_512M_r%%I.log 2>&1
)
