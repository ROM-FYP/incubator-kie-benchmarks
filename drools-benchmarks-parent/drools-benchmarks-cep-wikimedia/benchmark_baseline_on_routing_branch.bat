...
for %%M in (%MEMORY_CAPS%) do (
    echo ========================================
    echo Testing Baseline with -Xmx%%MM
    echo ========================================
    
    echo [BASELINE %%MM] Warmup...
    for /L %%W in (1,1,%WARMUP_ITERATIONS%) do (
        echo   Warmup %%W/%WARMUP_ITERATIONS%...
        start /affinity %CPU_AFFINITY% /B /WAIT java -Xms%%MM -Xmx%%MM ^
             -Xlog:gc*:file=selective_routing_results/gc_baseline_%%MM_w%%W.log:time ^
             -cp %JAR% %MAIN% replay %EVENT_FILE% false ^
             > selective_routing_results/baseline_%%MM_w%%W.log 2>&1
    )
    
    echo [BASELINE %%MM] Measurement...
    for /L %%I in (1,1,%MEASURE_ITERATIONS%) do (
        echo   Run %%I/%MEASURE_ITERATIONS%
        start /affinity %CPU_AFFINITY% /B /WAIT java -Xms%%MM -Xmx%%MM ^
             -Xlog:gc*:file=selective_routing_results/gc_baseline_%%MM_r%%I.log:time ^
             -cp %JAR% %MAIN% replay %EVENT_FILE% false ^
             > selective_routing_results/baseline_%%MM_r%%I.log 2>&1
    )
    echo.
)
...
powershell -Command "$caps = @('128', '256'); Write-Output 'Cap,Run,Events,Alerts,Duration_ms,Peak_MB,Max_MB,GC_Count,GC_Time_ms,Util_Pct' | Out-File selective_routing_results/baseline_results.csv -Encoding ASCII; foreach ($cap in $caps) { for ($i=1; $i -le 5; $i++) { $log = Get-Content \"selective_routing_results/baseline_${cap}M_r${i}.log\" -Raw; $events = if ($log -match 'Total Events Ingested: (\d+)') {$matches[1]} else {'0'}; $alerts = if ($log -match 'Total Terminal Alerts: (\d+)') {$matches[1]} else {'0'}; $duration = if ($log -match 'Duration: ([\d.]+) seconds') {[int]([double]$matches[1] * 1000)} else {'0'}; $peak = if ($log -match 'Peak Heap Used: (\d+) MB') {$matches[1]} else {'0'}; $max = if ($log -match 'Max Heap: (\d+) MB') {$matches[1]} else {'0'}; $gcCnt = if ($log -match 'GC Collections: (\d+)') {$matches[1]} else {'0'}; $gcTime = if ($log -match 'GC Time: (\d+) ms') {$matches[1]} else {'0'}; $util = if ($max -ne '0' -and $peak -ne '0') { [math]::Round(([double]$peak / [double]$max) * 100, 1) } else { 0 }; Write-Output \"$cap,$i,$events,$alerts,$duration,$peak,$max,$gcCnt,$gcTime,$util\" | Out-File selective_routing_results/baseline_results.csv -Append -Encoding ASCII } }; Import-Csv selective_routing_results/baseline_results.csv | Format-Table -AutoSize"

echo.
echo Complete! Baseline results in selective_routing_results/baseline_results.csv
endlocal
