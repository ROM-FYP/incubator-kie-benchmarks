 ...
 * under the License.
 */
package bench.opensky.analysis;
...
public class ForwardChainFinder {
    ...
    public ForwardChainResult findForwardChain(String entryFactType) {
        ...
        // Find SCC(s) containing seed rules — those with in-degree 0 start at depth 0
        ...
        for (int sccId = 0; sccId < sccCount; sccId++) {
            if (sccInDegree.get(sccId) == 0) {
                sccDepth.put(sccId, seedSccIds.contains(sccId) ? 0 : 0);
                topoQueue.add(sccId);
            }
        }
